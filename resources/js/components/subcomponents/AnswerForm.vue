			...
			<form @submit.prevent="add"  @keydown="form.onKeydown($event)">
			    <div class="answer-form-container">
				    ...
				    <div class="form-group data-input">
				      ...
				      <textarea v-model="form.text" type="text" name="text"
				        class="form-control" :class="{ 'is-invalid': form.errors.has('text') }" placeholder="Type your answer here.."></textarea>
				      ...
				      <div class="row">
				      	...
				      	<div class="col social-login">
				      		<a href="../facebook-login">
				      			<div><i class="fa fa-facebook-square" aria-hidden="true"></i> Login with Facebook</div>
				      		</a>
				      	</div>
				      </div>
				    </div>
	...
	export default {
		...
		methods: {
		    add() {
		      ...
		      this.form.post( siteUrl + '/api/answer')
		        .then((response) => { 
		        	// Hide progress bar
