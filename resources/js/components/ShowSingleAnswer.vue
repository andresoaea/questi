<template>
  <div class="answers">
    ...
    <div
      ...
      :class="answerWrapperClass(answer.id)"
      :id="`answer${answer.id}`"
    >
      ...
      <div class="user-question">
        ...
        <footer>
          ...
          <span v-tooltip:top="fullnameTooltip(answer)" class="float-left">
            <i class="fa fa-user-o" aria-hidden="true"></i>
            &nbsp; {{ getAuthorName(answer, 'firstname') }}
          </span>
...
export default {
  ...
  created() {
    ...
    axios.get(siteUrl + "/api/answers/" + this.questionId).then(response => {
      ...
      setTimeout(() => {
        if (window.location.hash) {
          $("html, body")
            .stop()
            .animate({ scrollTop: $(window.location.hash).offset().top });
        }
      }, 400);
    });
  ...
  methods: {
    ...
    fullnameTooltip: function(answer) {
      return (
        "Answered by: " +
        this.getAuthorName(answer, "firstname") +
        " " +
