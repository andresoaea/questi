<template>
  <div>
    <header class="section-title">
      <p class="float-left">{{ user.firstname }}'s answers</p>
    ...
    <table class="table user-data-table answers-list-table">
      <tr v-for="answer in answersData.data">
        <td class="users-image">
          <img
            v-tooltip:top="getFullName(user)"
            :src="user.picture | setImageSrc('profile')"
            class="img-circle"
          />
          <i class="fa fa-forward" aria-hidden="true"></i>
          <img
            v-tooltip:top="getFullName(answer.related_author)"
            :src="answer.related_author.picture | setImageSrc('profile')"
            class="img-circle"
          />
        </td>
        <td>
...
export default {
  ...
  methods: {
    ...
    getResults(page = 1) {
      axios
        .get(siteUrl + "/api/profile/data/answers?page=" + page)
        .then(response => {
