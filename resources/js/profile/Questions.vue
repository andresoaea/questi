<template>
	<div>
        ...
        <table class="table user-data-table">
            <tbody>
              ...
              <tr v-for="question in questionsData.data">
                ...
                <td>
                	<a :href="questionLink(question.slug, question.category_slug)" target="_blank"> <i aria-hidden="true" class="fa fa-question"></i> {{ question.title }}</a>
                </td>
                <td>
                  	{{ question.created_at | relativeTime('D MMM YYYY') }}
                </td>
                <td>
                	<span :class="['badge', question.answers_count > 0 ? 'bg-success' : 'bg-warning']">
                		{{ question.answers_count }} <i aria-hidden="true" class="fa fa-reply"></i>
                	</span>
                </td>
                <td v-if="question.best_answer">
                	<p class="text-success"><i class="fa fa-check-circle-o" aria-hidden="true"></i> Solved</p>
                </td>
                <td v-else>
                	<p class="text-danger"><i class="fa fa-check" aria-hidden="true"></i> In progress</p>
                </td>
                <td>
		...
		<pagination class="float-right" :limit="2" :data="questionsData" @pagination-change-page="getResults"></pagination>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getResults(page = 1) {
				axios.get(siteUrl + '/api/profile/data/questions?page=' + page)
					.then(response => {
